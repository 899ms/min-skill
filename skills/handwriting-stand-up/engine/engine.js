...
(() => {
  ...
  /* ------------------------------------------------------------------ */
  const Sound = (() => {
    let ctx = null, out, sfxBus, music, noiseBuf, enabled = true, sched = null, musicOn = false, drum = false, step = 0, nextT = 0, stopToken = 0;
    const cache = {};
    function init() {
    ...
    function musicStart(level = 0.55) {
      if (!ctx) return;
      stopToken++;
      if (!sched) { step = 0; nextT = ctx.currentTime + 0.08; sched = setInterval(tick, 40); }
      musicOn = true;
      music.gain.cancelScheduledValues(ctx.currentTime);
      music.gain.setTargetAtTime(level, ctx.currentTime, 0.6);
    }
    function musicLevel(level, tc = 0.5) { if (ctx) { music.gain.cancelScheduledValues(ctx.currentTime); music.gain.setTargetAtTime(level, ctx.currentTime, tc); } }
    function musicStop(tc = 0.8) {
      if (!ctx) return;
      musicOn = false;
      music.gain.cancelScheduledValues(ctx.currentTime);
      music.gain.setTargetAtTime(0, ctx.currentTime, tc);
      const my = ++stopToken;
      setTimeout(() => { if (my === stopToken && !musicOn && sched) { clearInterval(sched); sched = null; } }, tc * 5000);
    }
    return {
      init, play, musicStart, musicStop, musicLevel,
      setDrum(v) { drum = v; },
